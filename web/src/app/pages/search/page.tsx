...
export default function SearchPage() {
  ...
  const mode = decodeURIComponent(searchParams.get("mode") || "");
  const conversation_id: number = parseInt(decodeURIComponent(searchParams.get("conversation_id") || "0"), 10);
  return (
    <div className="absolute inset-0 bg-[url('/bg.svg')]">
      ...
      <div className="mx-auto max-w-3xl absolute inset-4 md:inset-8 bg-white">
        ...
        <div className="px-4 md:px-8 pt-6 pb-24 rounded-2xl ring-8 ring-zinc-300/20 border border-zinc-200 h-full overflow-auto">
          <Title query={query} mode={mode}></Title>
          <Result conversation_id={conversation_id} mode={mode} key={rid} query={query} rid={rid}></Result>
        </div>
